...
async def main2():
    ...
    try:
        ...
        if not df.empty:
            ...
            df_2=prepare_df_2(df)

            last_rows = df_2.iloc[-seq_len:]  # Adjust as needed
            last_row_scaled = scaler_X.transform(last_rows)  # Scale the data
            last_row_scaled = last_row_scaled.reshape(1, seq_len, -1)  # Reshape for model input

            # Make prediction
            future_prediction = model.predict(last_row_scaled)
